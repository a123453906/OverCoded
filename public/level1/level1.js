...
function draw() {
    ...
    for (let i = 0; i < code_segment.length; i++) {
        ...
        if((role1.overlap(button_submit)) && (keyIsDown(16)) || (role2.overlap(button_submit)) && (keyIsDown(69))){
            ...
            if(code_result.length == code_answer[level].length){
                if((check_array(code_result,code_answer[level])) || (check_array(code_result,level1_extra))){
                    //new
                    sSuccess = true;
                    //new
                    console.log('Correct');
    ...
    if (distance1 < 300 && distance1 > 150 && (distance1 < distance2)&&!ghost_faint) {
        ...
        if(ghost_faint){
            ...
            if(ghost1.position.x < 500 && ghost1.position.y < 100){
                ghostRMX = -1;
                ghostRMY = -1;
            }
    ...
    if(gameState == 'failed'){
        ...
        restartButton.size(250, 125);
        restartButton.mousePressed(restart);
        restartButton.position(730, 550);
    }
    // 修改的ending
